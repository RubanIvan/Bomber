...
public class TextureAtlasSlicer : EditorWindow {
    ...
    private void ParseXML() {
        try {
            ...
            var root = document.SelectNodes("/img/definitions/dir");
            if (root == null || root.Count != 1) {
                return;
            }

            //subTextures = root.ChildNodes
            subTextures = root[0].ChildNodes
                              .Cast<XmlNode>()
                              .Where(childNode => childNode.Name == "spr")
                              .Select(childNode => new SubTexture {
                                  width = Convert.ToInt32(childNode.Attributes["w"].Value),
                                  height = Convert.ToInt32(childNode.Attributes["h"].Value),
